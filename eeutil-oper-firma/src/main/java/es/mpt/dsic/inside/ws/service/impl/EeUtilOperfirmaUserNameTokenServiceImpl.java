...
public class EeUtilOperfirmaUserNameTokenServiceImpl implements
		EeUtilOperFirmaUserNameTokenService {

	protected final static Log logger = LogFactory
			.getLog(EeUtilOperfirmaUserNameTokenServiceImpl.class);
	...
	@Override
	public ResultadoValidacionInfo validacionFirma(byte[] firma,
			String tipoFirma, DatosFirmados datosFirmados)
		...
		ResultadoValidacionInfo validaFirma = AfirmaModelToWsModelConverter
				.resultadoValidacionInfoAfirmaToResultadoValidacionInfo(afirmaService
						.validarFirma(credentialUtil
								.getCredentialEeutilUserToken(wsContext)
								.getIdApplicacion(), firmaElectronica64,
								datos64, hash64, algoritmo, tipoFirma));
	...
	public InformacionFirma obtenerInformacionFirma(byte[] firma,
		...
		try {
			InformacionFirmaAfirma infoAfirma = afirmaService
					.obtenerInformacionFirma(credentialUtil
							.getCredentialEeutilUserToken(wsContext)
							.getIdApplicacion(), firma, opciones
							.isObtenerFirmantes(), opciones
							.isObtenerDatosFirmados(), opciones
							.isObtenerTipoFirma(), content);
	...
	@Override
	public ListaFirmaInfo obtenerFirmantes(byte[] firma, byte[] datos,
		...
		InformacionFirma informacionFirma = this.obtenerInformacionFirma(firma,
				opciones, null);
	 ...
	 */
	@Override
	public ResultadoValidarCertificado validarCertificado(String certificate)
			throws InSideException {
		return AfirmaModelToWsModelConverter
				.resultadoValidarCertificadoAfirmaToResultadoValidarCertificado(afirmaService
						.validarCertificado(credentialUtil
								.getCredentialEeutilUserToken(wsContext)
								.getIdApplicacion(), certificate, false));
	}
	 ...
	 */
	@Override
	public InfoCertificado getInfoCertificado(String certificate)
			throws InSideException {
		return AfirmaModelToWsModelConverter
				.resultadoValidarCertificadoAfirmaToResultadoValidarCertificadoAmpliado(afirmaService
						.validarCertificado(credentialUtil
								.getCredentialEeutilUserToken(wsContext)
								.getIdApplicacion(), certificate, true));
	}
	 ...
	 */
	@Override
	public ResultadoAmpliarFirma ampliarFirma(byte[] firma,
		...
		try {
			...
			ResultadoAmpliarFirmaAfirma resAfirma = afirmaService
					.ampliarFirma(
							credentialUtil.getCredentialEeutilUserToken(
									wsContext).getIdApplicacion(),
							firma,
		...
		try {
			...
			ResultadoValidacionFormatoAInfo resultadoValidar = AfirmaModelToWsModelConverter
					.resultadoValidacionFirmaFormatoAAfirmaToResultadoValidacionFormatoAInfo(afirmaService
							.validarFirmaFormatoA(credentialUtil
									.getCredentialEeutilUserToken(wsContext)
									.getIdApplicacion(), XMLUtil
									.decodeUTF8(firmaValidar), null, null,
	...
	@Override
	public ResultadoAmpliarFirma resellarFirmaA(byte[] firma)
			throws InSideException {
		try {
