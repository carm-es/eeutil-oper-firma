...
public class EeUtilServiceMtomImpl implements EeUtilServiceMtom {
	...
	public ResultadoValidacionInfo validacionFirma(ApplicationLogin info,
			DataHandler firma, String tipoFirma,
			DatosFirmadosMtom datosFirmadosMtom) throws InSideException {
		try {
			...
			if (datosFirmadosMtom != null
					&& datosFirmadosMtom.getDocumento() != null) {
				documentoBytes = IOUtils.toByteArray(datosFirmadosMtom
						.getDocumento().getInputStream());
			}

			byte[] hashBytes = null;
			if (datosFirmadosMtom != null
					&& datosFirmadosMtom.getHash() != null) {
				hashBytes = IOUtils.toByteArray(datosFirmadosMtom.getHash()
						.getInputStream());
			}
			...
			if (datosFirmadosMtom != null
					&& StringUtils.isNotEmpty(datosFirmadosMtom.getAlgoritmo())) {
			...
			return eeUtilService.validacionFirma(info, firmaBytes, tipoFirma,
					datosFirmados);
		} catch (IOException e) {
			logger.error("Error en peticion EeUtilServiceMtomImpl.validacionFirma:"
					+ e.getMessage());
			throw new InSideException(e.getMessage(), new EstadoInfo("ERROR",
					"ERROR INESPERADO AL VALIDAR FIRMA", null), e);
		}
	}

	/**
	 * Obtiene la informaci�n de una firma, a partir de una llamada a los WS DSS
	 * de Afirma
	 * 
	 * @param info
	...
	public InformacionFirma obtenerInformacionFirma(
			ApplicationLogin aplicacion, DataHandler firma,
		...
		try {
			...
			return eeUtilService.obtenerInformacionFirma(aplicacion,
					firmaBytes, opciones, contenidoBytes);
		} catch (IOException e) {
			logger.error("Error en peticion EeUtilServiceMtomImpl.obtenerInformacionFirma:"
					+ e.getMessage());
			throw new InSideException(e.getMessage(), new EstadoInfo("ERROR",
	...
	@Override
	public ListaFirmaInfo obtenerFirmantes(ApplicationLogin info,
			DataHandler firma, DataHandler datos, String tipoFirma)
			throws InSideException {
		try {
			byte[] firmaBytes = null;
			if (firma != null) {
			...
			return eeUtilService.obtenerFirmantes(info, firmaBytes, datosBytes,
					tipoFirma);
		} catch (IOException e) {
			logger.error("Error en peticion EeUtilServiceMtomImpl.obtenerInformacionFirmas:"
					+ e.getMessage());
			throw new InSideException(e.getMessage(), new EstadoInfo("ERROR",
					"ERROR INESPERADO AL OBTENER INFORMACION DE FIRMA", null),
					e);
	...
	/**
	 * Recibe una firma y una configuraci�n de ampliaci�n y devuelve la
	 * firma con el upgrade apropiado
	 */
	@Override
	public ResultadoAmpliarFirmaMtom ampliarFirma(ApplicationLogin info,
			DataHandler firma, ConfiguracionAmpliarFirma configuracion)
			throws InSideException {
		try {
			byte[] firmaBytes = null;
			if (firma != null) {
			...
			ResultadoAmpliarFirma resultado = eeUtilService.ampliarFirma(info,
					firmaBytes, configuracion);
			...
			logger.error("Error en peticion EeUtilServiceMtomImpl.ampliarFirma:"
					+ e.getMessage());
			throw new InSideException(e.getMessage(), new EstadoInfo("ERROR",
					"ERROR INESPERADO AL AMPLIAR FIRMA", null), e);
		}
	}

	@Override
	public DataHandler postProcesarFirma(ApplicationLogin info,
		...
		try {
			...
			logger.error("Error en peticion EeUtilServiceMtomImpl.postProcesarFirma:"
					+ e.getMessage());
			throw new InSideException(e.getMessage(), new EstadoInfo("ERROR",
					"ERROR INESPERADO AL POST PROCESAR FIRMA", null), e);
		}
	}
	...
	public ResultadoComprobarFirmaFormatoA comprobarFirmaFormatoA(
			ApplicationLogin info, DataHandler firma) throws InSideException {
		try {
			...
			String tipoFirmaDss = afirmaService.obtenerTipoFirmaDss(
					info.getIdApplicacion(), firmaBytes, null);
			...
			ResultadoValidacionFormatoAInfo resultadoValidar = AfirmaModelToWsModelConverter
					.resultadoValidacionFirmaFormatoAAfirmaToResultadoValidacionFormatoAInfo(afirmaService
							.validarFirmaFormatoA(info.getIdApplicacion(),
									XMLUtil.decodeUTF8(firmaBytes), null, null,
									null, formatoValidacion));
			...
			logger.error(
					"Error en EeUtilServiceMtomImpl.comprobarFirmaFormatoA", e);
			EstadoInfo estadoInfo = new EstadoInfo("ERROR", e.getCode(),
					e.getMessage());
			throw new InSideException(estadoInfo.getDescripcion(), estadoInfo);
		} catch (IOException e) {
			logger.error("Error en peticion EeUtilServiceMtomImpl.comprobarFirmaFormatoA:"
					+ e.getMessage());
			throw new InSideException(e.getMessage(), new EstadoInfo("ERROR",
					"ERROR INESPERADO AL POST PROCESAR FIRMA", null), e);
		}
	...
	@Override
	public ResultadoAmpliarFirmaMtom resellarFirmaA(ApplicationLogin info,
			DataHandler firma) throws InSideException {
		try {
			...
			logger.error("Error en peticion EeUtilServiceMtomImpl.resellarFirmaA:"
					+ e.getMessage());
			throw new InSideException(e.getMessage(), new EstadoInfo("ERROR",
					"ERROR INESPERADO AL RESELLAR FIRMA", null), e);
		}
	}
}
