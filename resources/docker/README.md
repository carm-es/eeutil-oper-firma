...
```sh
$ sudo docker run --name eeutil-oper-firma-c -v <path_config>:/usr/local/tomcat/conf/eeutil -v <path_logs>:/usr/local/tomcat/logs -p 8080:8080 eeutil-oper-firma-i
```

Breve resumen sobre cada parámetro de la ejecución:
* ``--name eeutil-oper-firma-c``: Nombre del contenedor 
...
Puede comprobar que el contenedor esta ejecutándose correctamente accediendo mediante un navegador a la ruta http://localhost:8080/eeutil-oper-firma/ws
